...
public class EmulatorB : IEmulator
{
    private readonly ConnectionFactory _connectionFactory;
    private readonly IConnection _connection;
    private readonly IModel _channel;
    private readonly RabbitMQConfig _rmqConfig;
    private readonly ILogger<EmulatorB> _logger;

    public EmulatorB(ILogger<EmulatorB> logger, RabbitMQConfig rmqConfig)
    {
    ...
    public async Task StartAsync(CancellationToken stoppingToken, int instance)
    {
        await Task.Run(async () =>
        {
            try
            {
                _logger.LogInformation($"EmulatorB {instance} doing work");
                ...
                consumer.Received += (model, ea) =>
                {
                    ...
                    _logger.LogInformation($"EmulatorB {instance} processing message");
            ...
            {
                _logger.LogError(ex, "An error occurred while processing messages in EmulatorB.");
            }
        }, stoppingToken);
    }
}