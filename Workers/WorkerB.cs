...
public class WorkerB : IWorker
{
    ...
    public WorkerB(ILogger<WorkerB> logger, RabbitMQConfig rmqConfig)
    {
    ...
    public async Task StartAsync(CancellationToken stoppingToken, int instance)
    {
        await Task.Run(async () =>
        {
            try
            {
                _logger.LogInformation($"WorkerB {instance} doing work");
                ...
                consumer.Received += (model, ea) =>
                {
                    ...
                    _logger.LogInformation($"WorkerB {instance} processing message");
            ...
            {
                _logger.LogError(ex, "An error occurred while processing messages in WorkerB.");
            }
        }, stoppingToken);
    }
}